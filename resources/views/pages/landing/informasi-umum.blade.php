    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-3 d-none d-lg-block">
                <div class="sidebar-nav position-sticky" style="top: calc(76px + 0.025 * (100vh - 76px));">
                    <div class="d-flex gap-0">
                        <x-informasi-links :allKategori="$allKategori" :currentKategori="$kategoriModel->kategori" /> 
                    </div>        
                </div>
            </div>
            ...
            <div class="col-lg-9 ps-lg-5">
                <h2 class="fw-bold mb-4 pt-4 w-100 text-center">{{ $kategoriModel->kategori }}</h3>

                @foreach($kategoriModel->informasiUmum as $info)
                    <h4 class="fw-bold mb-4 pt-4" id="info-{{ Str::slug($info->judul) }}">{{ $info->judul }}</h4>
                    <p class="text-muted text-sm">Debug Kategori: {{ $info->kategori->kategori }}</p>
